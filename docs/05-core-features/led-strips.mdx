...
Addressable LED strips allow each LED in the strip to be programmed with a unique and independent color.
...
The data line (LED pad) from the flight controller will have to be used.
...
Note: this mode requires a current sensor. If you don't have the actual device you can set up a virtual current sensor (see [Battery](./battery.mdx)).
...
Each LED of the ring can be a different color. The color can be selected between the 16 available colors.
