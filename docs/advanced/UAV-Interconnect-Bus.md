...
During discovery phase master sends _IDENTIFY_ commands for each supported **DevID**.
Device with corresponding **DevID** must respond with desired poll interval (in milliseconds) and flag field.
Master will send it's protocol version in _IDENTIFY_ request. Slave device should respond only if it's able to talk this protocol version.
