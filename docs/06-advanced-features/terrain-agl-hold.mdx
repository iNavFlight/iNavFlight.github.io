---
title: Terrain AGL Hold
description: Fixed-wing terrain following — hold a set height above the ground, not above the take-off point
---
...
![How Terrain AGL Hold works](/img/features-adv/terrain-agl-hold-diagram.png)
...
4. **OSD:** add the **Terrain AGL** element to see your height above ground (AGL) in flight. It always shows the value from the terrain map. The **Rangefinder** element works too, but if a physical rangefinder is fitted it shows the rangefinder instead. Either way Terrain AGL Hold flies from its map and its alarms are unaffected. The flight-mode field shows **TERR** while the hold is engaged.
...
|---|---|---|---|
| [`terrain_nav_min_agl`](https://github.com/iNavFlight/inav/blob/maintenance-10.x/docs/Settings.md#terrain_nav_min_agl) | 60 m | 50 – 120 m | The **safety floor** — the lowest height above ground the mode will hold. Engage below it and it climbs up to ~10 m above it. (Stored in centimetres: 6000 = 60 m.) |
| [`terrain_nav_lookahead`](https://github.com/iNavFlight/inav/blob/maintenance-10.x/docs/Settings.md#terrain_nav_lookahead) | 1000 m | 0 – 2000 m | How far ahead along your track the **forward scan** looks for rising terrain. The scan is additionally capped at **35 s of flight at your current ground speed**, because the horizon is really *time*, not distance: the same 1000 m is nearly a minute of flying at 20 m/s but only ~18 s at 55 m/s. The time cap stops slow flight from climbing absurdly early for terrain a minute away, while fast flight still uses the full distance. `0` disables the early *TERRAIN AHEAD!* warning, leaving only the reactive floor. |

:::warning
**Keep your climb rates honest.** The climb authority is your existing **`nav_fw_auto_climb_rate`** — there is no separate terrain climb setting, and the *TERRAIN AHEAD!* escape warning **trusts that number**, so set it to a rate your model genuinely sustains. Keep **`nav_fw_manual_climb_rate` at least equal to `nav_fw_auto_climb_rate`** (they default to 300 / 500) — or **greater**, if you want pulling the stick to actually add climb. If `nav_fw_manual_climb_rate` is **lower** than `nav_fw_auto_climb_rate`, grabbing pitch during an auto-climb commands the lower rate: the nose visibly eases and it can even **slow** the climb.
:::
...
And this is a hands off climb over a rising hill. The height is set to 80 m and the stick stays untouched. The mode climbs the plane at 4.9 m/s to follow the slope. The AGL readout shows 67 m, a little under the set height, which is normal on a steep slope because the climb rate is limited. The altitude readout shows 197 m above the take-off point. That is not an error. The ground under the plane has simply risen, so the plane must fly higher to keep the same height above it. There is no alarm, because above the floor the mode does its job quietly.

![Hands-off climb over a rising hill in Terrain AGL Hold](/img/features-adv/terrain-agl-hold-over-hill.jpg)
...
Here is the ladder in a real flight. The two pictures are six seconds apart. In the first one the plane is flown down on purpose to 23 m above the ground. **TERRAIN! PULL UP!** appears, meaning the plane is too low but a climb will still save it. In the second one the stick is released. The autopilot takes over right away and starts climbing back (the vario shows +6.1 m/s), and the OSD says what it is doing with **TERRAIN AUTO CLIMB TO MIN**.

![TERRAIN! PULL UP! during a deliberate descent](/img/features-adv/terrain-agl-hold-pull-up.jpg)

![TERRAIN AUTO CLIMB TO MIN, the autopilot climbing back after the stick is released](/img/features-adv/terrain-agl-hold-auto-climb.jpg)
