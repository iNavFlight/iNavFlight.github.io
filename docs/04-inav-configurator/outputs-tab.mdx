...
The Outputs tab allows you to adjust the parameters of each of the physical outputs of your aircraft.
In the Mixer Tab, you indicate how the sticks on your transmitter are mapped to the multiple servos and motors.
The Outputs Tab lets you set parameters for each individual servo and the communication protocol for your motors.

![Outputs Tab](/img/gettingstarted/inavconfig_outputs.png)
...
ESC protocol can be selected in Configurator in the dropdown menu.
No special configuration is required.

Check the ESC documentation for the list of protocols that are supported.

- Stop motors on low throttle: when throttle is at the lowest position, the motor will be stopped.
- Motor IDLE Power (%): is the minimum PWM value the motor sees when running at its lowest setting.
...
3. If your vehicle oscillates for several seconds or even continues oscillating indefinitely, then the filter cutoff frequency should be reduced. Reduce the value of `servo_lpf_hz` by half its current value and repeat the previous step.
