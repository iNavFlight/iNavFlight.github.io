...
### Arming Override
Arming is disabled when Nav modes are configured and no GPS lock is available **or** if a waypoint mission is loaded but the first WP is farther than the `nav_wp_safe_distance` setting.
